...
export const headDataSchema = z.object({
  [HEAD_DATA_FORM.INPUTS.FULL_NAME.KEY]: z.string()
    .min(2, { message:ERROR.HEAD_DATA.SHORT})  // Validación de longitud mínima
    .regex(nameRegex, { message:ERROR.HEAD_DATA.ONLY_ALPHABETICAL}), // Validación de caracteres permitidos
    
    [HEAD_DATA_FORM.INPUTS.PHOTO.KEY]: z
    .any()
    .refine(async (file) => {
      if (!file) return true
